...
```{r,echo=F,fig.width=9, fig.height=4,warning=F}
dirs <- list.dirs(path = ".", full.names = TRUE, recursive = TRUE) 

r.target <- paste(c(".git",".Rproj.user",".quarto"),
                  collapse='|')
dirs <- dirs[!grepl(r.target,dirs)]

e.target <- c("./data","./figure","./metadata","./output","./scripts")
dirs <- 
  purrr::map(e.target,~{
...
x <- lapply(strsplit(dirst, "/"), 
  ...
  reframe(V4 = case_when(n() > 8~sprintf("%s files",n()),
                           T~V4)) %>% 
  mutate(V2=factor(V2,levels=c("data","metadata","scripts","output","figure","docs"))) %>% 
...
do.call('rbind', 
  ...
  geom_node_point(shape = 21, size=2,fill = 'lightblue') +
  geom_node_text(aes(label = label), size = 2.5, nudge_x = 0.5) +
  coord_flip(clip = 'off') +
  scale_y_reverse() +
  theme_graph()
